...
| # | Flow | FE | BE | Verdict | Gap to close before it passes |
|---|---|---|---|---|---|
| **E1** | Create company account | ◑ | ✅ | ◑ **Partial** | Create form → `POST /employer/profile` is real; company screen loads. **But** verification still uses a local seam (`peoplor_employer_verify_v1` + a demo "approve" button) and a company-extras seam (`peoplor_company_extras_v1`). Backend verification/resubmit + admin approve **are built**, and company **voice-parse is now wired** (2026-07-03): a "Speak to autofill" panel on the company form → `POST /employer/profile/parse` → prefills company fields. **Remaining E1 gap:** drop the verification/company-extras local seams and read `verificationStatus`. |
