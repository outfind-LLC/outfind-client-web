...
| **E3** | Post a job — manual | ✅ | ✅ | ✅ **Built (2026-07-04)** | Wizard (regular 5-step + daily 3-step, per-design steps rail + progress bar + sticky footer, responsive) → `POST /employer/vacancies` sends the **full field set** (kind, profession, category, address, workFormat, workArrangement, workSchedule, teamSize, paymentType/Frequency/Note, workDate+shiftHours w/ date guard, educationRequired, probationMonths, languageRequirements, visibility, acceptResponses, description). **AI description is now REAL**: `POST /employer/vacancies/generate-description` writes it from the wizard's structured facts (was a canned local template); employer edits before saving. Drafts: save → DRAFT (invisible to workers) → publish from Vacancies › Closed › kebab. Docs: `docs/features/vacancy-wizard.md`. |
